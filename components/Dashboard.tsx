...
const Dashboard: React.FC<DashboardProps> = ({ recentInvoices, isOffline, pendingCount, onNewAction, onSelectInvoice, onNavigate, currentUser }) => {
   ...
   const stats = useMemo(() => {
      ...
      const monthlyRevenue = thisMonthInvoices.reduce((acc, curr) => acc + (curr.baseAmountEur || curr.total), 0);
            ...
            <div>
               ...
               <div className="space-y-4">
                  {recentInvoices.slice(0, 5).map((inv) => (
                     ...
                     >
                        {/* Icon Box */}
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors ${inv.type === 'Quote'
