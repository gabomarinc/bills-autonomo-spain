    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        if (hasAiAccess) {
          ...
          if (fileType === 'image') {
            setIsLoading(true);
            setLoadingMsg('Escaneando texto (OCR Local)...');
            try {
              const ocrResult = await scanReceipt(file);
              setExpenseData({
                clientName: '', // Hard to get from raw text reliably without LLM
                amount: ocrResult.total || 0,
                currency: currentUser.defaultCurrency || 'EUR',
                concept: file.name.split('.')[0], // Default to filename
                date: ocrResult.date || new Date().toISOString().split('T')[0],
                isDeductible: !!ocrResult.nif, // If we found a NIF, likely deductible
                isValidDoc: !!ocrResult.nif
              });
              // If we found a NIF, we might want to store it, but ExpenseData doesn't have it yet.
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-30">
              ...
              <div className="bg-white/90 p-8 rounded-[2rem] shadow-2xl flex flex-col items-center backdrop-blur-md border border-white/50 animate-in zoom-in-95">
                ...
                <p className="text-slate-400 text-xs mt-2 uppercase tracking-widest font-bold">
                  {hasAiAccess ? 'Potenciado por Gemini IA' : 'Procesamiento IA Local'}
                </p>
              </div>
            </div>
          ) : (
