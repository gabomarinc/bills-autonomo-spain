...
type DocStage = 'ALL' | 'DRAFT' | 'ALL_ACTIVE' | 'TO_COLLECT' | 'NEGOTIATION' | 'SENT_QUOTES' | 'DONE';
...
const DocumentList: React.FC<DocumentListProps> = ({
   ...
   const [currentStage, setCurrentStage] = useState<DocStage>('ALL'); // Default to ALL
   ...
   useEffect(() => {
      // Always default to ALL when switching types for better UX
      setCurrentStage('ALL');
   }, [docTypeFilter]);
   ...
   const filteredDocs = invoices.filter(doc => {
      ...
      if (currentStage === 'ALL') return true; // Show everything for this doc type
                  ...
                  >
                     ...
                     <div className="p-6 flex-1 flex flex-col justify-between relative z-10">
                        ...
                        <div className="flex items-center gap-3">
                           <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${doc.type === 'Quote' ? 'bg-purple-50 text-purple-500' : 'bg-slate-50 text-slate-500'
                              }`}>
                  ...
                  {filteredDocs.map((doc) => (
                     <div key={doc.id} onClick={() => onSelectInvoice(doc)} className="bg-white p-4 rounded-2xl border border-slate-50 shadow-sm flex items-center justify-between">
                        ...
                        <div className="text-right">
                           ...
                           <span className={`text-[10px] font-bold uppercase ${doc.status === 'Aceptada' || doc.status === 'Pagada' ? 'text-green-600' :
